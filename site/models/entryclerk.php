 ...
 * @subservice	com_toes
 */
class TOESModelEntryclerk extends JModelList {
    ...
    public function getShowDetails($pk = null) {
        ...
        $query->select('s.show_format ,s.show_id, s.show_start_date, s.show_end_date, s.show_flyer, s.show_motto, s.show_status, s.catalog_runs, s.show_extra_text_for_confirmation, s.show_paper_size, s.show_currency_used, s.show_comments, s.show_cost_per_entry, s.show_total_cost, s.show_uses_toes,s.show_uses_ticapp, s.show_bring_your_own_cages');
...
//spiderweb commented
    public function getMarkedCatalogPDF($show_id)
    {
        $user = JFactory::getUser();
		$app = JFactory::getApplication();
        ...
        if(TOESHelper::is_showofficial($user->id, $show_id) || TOESHelper::is_clubowner($user->id, TOESHelper::getClub($show_id)->club_id) || TOESHelper::isAdmin())
        {
			$error = '';
			ob_start();
            require_once JPATH_COMPONENT.DS.'pdflibs'.DS.'MarkedCatalog.php';
			$error = ob_get_contents();
			ob_end_clean();
			if(!$error)
				echo '1';
