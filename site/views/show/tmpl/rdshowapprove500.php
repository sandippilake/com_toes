...
$approval = $this->rdshowapproveurl;
$showrejected = $this->showsrejected;
...
<div id="toes">
	...
	if(!empty($approval))
		...
		if(empty($approval) && (empty($showrejected)))
		{
		...
		{
			JFactory::getApplication()->enqueueMessage('Shows are rejected by '.$showrejected->club_name);
		}
		?>
...
function rdapproveshow(id,hash)
{
	...
	jQuery.ajax({
			method: "POST",
			url: "index.php?option=com_toes&task=show.rdapproveshow&id="+id+"&hash="+hash+"&tmpl=component",	
			data: {data:str},
...
function rddisapproveshow(id,hash)
{
	...
	jQuery.ajax({
			method: "POST",
			url: "index.php?option=com_toes&task=show.rddisapproveshow&id="+id+"&hash="+hash+"&tmpl=component",	
			data: {data:str},
			success:function(data){
