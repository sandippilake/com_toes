...
function write($phpWord, $writers,$show_id)
{
	...
	//	$result .= date('H:i:s') . " Write to {$writer} format";
		if (!is_null($extension)) {
			...
			$xmlWriter->save(JPATH_BASE. "/media/com_toes/DOCX/".$show_id."/late_pages.docx");
...
//\PhpOffice\PhpWord\Shared\Html::addHtml($section, $html);
	...
	foreach ($entries as $entry) {
       ...
       $nhcell1->addText(JText::_('COM_TOES_LATE_PAGES'), $fontStyleTitle, array_merge($paragraphStyleTitle, array('align' => 'left')));
       ...
       $newheader->addLine($lineStyle);
		...
		while ($print_block > 1) {
			...
			/*if ($pdf->getPage() == $block_page) {
				...
				$show_class_block .= '<span style="font-size:40px; font-weight:bold;">' . JText::_('COM_TOES_LATE_PAGES') . '</span><br/><span style="font-size:35px">' . JText::_('COM_TOES_WEBSITE') . '</span>';
				$show_class_block .='</td>';
...
if(file_exists(JPATH_BASE. "/media/com_toes/DOCX".DS.$show_id.DS.'late_pages.docx'))
	unlink (JPATH_BASE. "/media/com_toes/DOCX".DS.$show_id.DS.'late_pages.docx');
