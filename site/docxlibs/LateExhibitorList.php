...
$writers = array('Word2007' => 'docx');
		...
		function write($phpWord, $writers,$show_id)
		{
			...
			//	$result .= date('H:i:s') . " Write to {$writer} format";
				if (!is_null($extension)) {
					...
					$xmlWriter->save(JPATH_BASE. "/media/com_toes/DOCX/".$show_id."/late_exibitor_list.docx");
       ...
       $cell->addText(JText::_('COM_TOES_LATE_EXHIBITOR_LIST'), $fontStyleTitle, array_merge($paragraphStyleTitle, array('align' => 'left')));
...
$db = JFactory::getDBO();
//$query = "SELECT `Exhibitor` , `Address` , `City` , `Country` , `Entries` , `show_id` , `late_entry` FROM `#__toes_view_exhibitor_list_basis` WHERE (`late_entry` = 1) AND (`show_id` = ".$show_id.")";
...
WHERE `e`.`late_entry` = 1 AND `e`.`entry_show` = $show_id AND ( (`es`.`entry_status` = 'Accepted') OR(`es`.`entry_status` = 'Confirmed') OR (`es`.`entry_status` = 'Confirmed & Paid') )
...
if(file_exists(JPATH_BASE. "/media/com_toes/DOCX".DS.$show_id.DS.'late_exibitor_list.docx'))
	unlink (JPATH_BASE. "/media/com_toes/DOCX".DS.$show_id.DS.'late_exibitor_list.docx');
