...
function write($phpWord, $writers,$show_id)
{
	...
	//	$result .= date('H:i:s') . " Write to {$writer} format";
		if (!is_null($extension)) {
			...
			$xmlWriter->save(JPATH_BASE. "/media/com_toes/DOCX/".$show_id."/mastercatalog.docx");
...
//\PhpOffice\PhpWord\Shared\Html::addHtml($section, $html);
	...
	foreach ($entries as $entry) {
       ...
       $nhcell1->addText(JText::_('COM_TOES_MASTER_CATALOG'), $fontStyleTitle, array_merge($paragraphStyleTitle, array('align' => 'left')));
	   ...
	   $newheadertable->addCell(9000,array('colspan'=>2))->addText('____',array('width'=>'9000'));*/
	   $newheader->addText();
				...
				foreach ($show_day_rings[$show_days[1]->show_day_id] as $second_show_day) {
					if ($show->show_format != 'Alternative') {
						if (isset($show_day_entries[$show_days[1]->show_day_id][$entry->catalog_number]))
							$table->addCell(700,array('align'=>'right','valign'=>'top'))->addText('____');
						else
						...
						if (isset($show_day_entries[$show_days[1]->show_day_id][$entry->catalog_number])) {
							...
							$table->addCell(700,array('align'=>(($second_show_day->ring_timing == 1) ? 'left' : 'right'),'valign'=>'top'))->addText('____');
							else
       ...
       $newheader->addLine($lineStyle);
		...
		if ($previous_catalog_number != $entry->catalog_number) {
			...
			$ntable2=$cell2->addTable(); 
		  	...
		  	$ntable2->addCell(3000)->addText( strtoupper($entry->catalog_cat_name)  ,null,array('align'=>'left'));
			...
			if (trim($entry->catalog_registration_number) == '')
				...
				$ntable2->addRow(200);
				$ntable2->addCell(5000,array('gridspan'=>3))->addText(strtoupper($reg_number) . '   ' . JText::_('COM_TOES_CATALOG_BORN') . ' ' . $entry->catalog_birthdate);
			
			if ($isNotHHP) {
				$ntable2->addRow(200);
				$ntable2->addCell(4000,array('gridspan'=>3))->addText(strtoupper($entry->catalog_sire));
				$cell2->addText(strtoupper($entry->catalog_dam));
			}
			//$ntable21=$cell2->addTable();
			//$ntable21->addRow(200);
			$cell3=$ntable2->addCell(4000,array('gridspan'=>3));
			if($entry->catalog_breeder)
				$cell3->addText( strtoupper($entry->catalog_breeder) );
			if($entry->catalog_owner)
				$cell3->addText(strtoupper($entry->catalog_owner));
			if($entry->catalog_lessee)
				$cell3->addText($entry->catalog_lessee);
			if($entry->catalog_agent)
				$cell3->addText($entry->catalog_agent);		
	    	$ntable2->addCell(700)->addText($entry->catalog_region);
					...
					foreach ($show_days as $show_day) {
						...
						foreach ($show_day_rings[$show_day->show_day_id] as $pr_show_day) {
							...
							if (isset($show_day_entries[$show_day->show_day_id][$entry->catalog_number]))
								$ntable3->addCell(500)->addText('____',null,array('align'=>'left'));
							else
								$ntable3->addCell(500)->addText('____',null,array('align'=>'left'));
					...
					foreach ($show_days as $show_day) {
						...
						if (isset($show_day_entries[$show_day->show_day_id][$entry->catalog_number])) {
								if (($pr_show_day->ring_timing == 1 && $showday_entries[$show_day->show_day_id][$entry->catalog_number]->entry_participates_AM) || ($pr_show_day->ring_timing == 2 && $showday_entries[$show_day->show_day_id][$entry->catalog_number]->entry_participates_PM))
									$ntable4->addCell(500)->addText('____',null,array('align'=>(($pr_show_day->ring_timing == 1) ? 'left' : 'right')));
								else
									$ntable4->addCell(500)->addText('____',null,array('align'=>(($pr_show_day->ring_timing == 1) ? 'left' : 'right')));
							
								
							} else
							$ntable4->addCell(500)->addText(' ',null,array('align'=>(($pr_show_day->ring_timing == 1) ? 'left' : 'right')));
		...
		while ($print_block > 1) {
			...
			if (in_array($entry->show_class, $skip_division_best)) {
				...
				$ntable4->addRow(500);
						...
						foreach ($show_day_rings[$show_day->show_day_id] as $pr_show_day) {
							...
							$ntable4->addCell(500)->addText('____',null,array('align'=>(($show->show_format != 'Alternative' || $pr_show_day->ring_timing == 1) ? 'left' : 'right')));
			...
			if (in_array($entry->show_class, $skip_breed_best)) {
				...
				for ($i = 1; $i <= $previous_breed_entries; $i++) {
					...
					foreach ($show_days as $show_day) {
						...
						foreach ($show_day_rings[$show_day->show_day_id] as $pr_show_day) {
							$ntable5->addCell(500,array('gridspan'=>2))->addText('____',null,array('align'=>(($show->show_format != 'Alternative' || $pr_show_day->ring_timing == 1) ? 'left' : 'right')));
			...
			/*if ($pdf->getPage() == $block_page) {
				...
				$show_class_block .= '<span style="font-size:40px; font-weight:bold;">' . JText::_('COM_TOES_MASTER_CATALOG') . '</span><br/><span style="font-size:35px">' . JText::_('COM_TOES_WEBSITE') . '</span>';
				$show_class_block .='</td>';
...
foreach ($final_entries as $ring_number => $cng_entries) {
	...
	foreach ($congress_entries as $entry) {
		...
		$table2->addRow();
			...
			$table2->addCell(1000,array('align'=>'right'))->addText('______',null,array('align'=>'right'));
				} else {
		...
		//var_dump($congress_entries[$next]);
			if ($entry->show_class != 'Ex Only' && in_array($entry->show_class, $skip_division_best)) {
				...
				for ($i = 1; $i <= $previous_division_entries; $i++) {
					...
					foreach ($rings as $ring) {
						$table4->addCell(1000)->addText('______',null,array('align'=>'right'));
			...
			if ($entry->show_class != 'Ex Only' && in_array($entry->show_class, $skip_breed_best)) {
				...
				for ($i = 1; $i <= $previous_breed_entries; $i++) {
					...
					foreach ($rings as $ring) {
						$table5->addCell(1000)->addText('______',null,array('align'=>'right'));
...
if(file_exists(JPATH_BASE. "/media/com_toes/DOCX".DS.$show_id.DS.'mastercatalog.docx'))
	unlink (JPATH_BASE. "/media/com_toes/DOCX".DS.$show_id.DS.'mastercatalog.docx');
