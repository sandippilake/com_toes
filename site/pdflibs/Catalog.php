...
$pdf->SetCreator("TICA");
$pdf->SetTitle(JText::_('COM_TOES_CATALOG_REPORT'));
...
if (count($show_days) == 2) {
	foreach ($entries as $entry) {
		if ($previous_class != $entry->show_class) {
			...
			$show_class_block .= '<span style="font-weight:bold;">' . JText::_('COM_TOES_CATALOG') . '</span><br/><span style="">' . JText::_('COM_TOES_WEBSITE') . '</span>';
			$show_class_block .='</td>';
...
foreach ($final_entries as $ring_number => $cng_entries) {
	...
	$show_class_block .= '<span style="font-weight:bold;">' . JText::_('COM_TOES_CATALOG') . '</span><br/><span>' . JText::_('COM_TOES_WEBSITE') . '</span>';
	$show_class_block .='</td>';
	//$show_class_block .='<td style="width:40%"><div style="text-align:center; font-weight:bold;font-size:70px">' . strtoupper($judge_name . '<br/>' . $entry->show_class) . '</div></td>';
...
if (file_exists(TOES_PDF_PATH . DS . $show_id . DS . 'catalog.pdf'))
	unlink(TOES_PDF_PATH . DS . $show_id . DS . 'catalog.pdf');

//Close and output PDF document
$pdf->Output(TOES_PDF_PATH . DS . $show_id . DS . 'catalog.pdf', 'F');
*/
$pdf->Output( $show_id .'_catalog.pdf', 'I');
