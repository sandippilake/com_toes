...
class MYPDF extends TCPDF {
	...
	public function Footer() {
		...
		$this->SetY($cur_y);
		// Set font
// setfontissue 		$this->SetFont('helvetica', 'I', 8);
...
// setfontissue $pdf->setHeaderFont(Array(PDF_FONT_NAME_MAIN, '', PDF_FONT_SIZE_MAIN));
// setfontissue $pdf->setFooterFont(Array(PDF_FONT_NAME_DATA, '', PDF_FONT_SIZE_DATA));

// set default monospaced font
// set fontissue $pdf->SetDefaultMonospacedFont(PDF_FONT_MONOSPACED);
...
// set font
// setfontissue $pdf->SetFont('ptsans', '', 12);
...
if($is_alternative)
{
	...
	LEFT JOIN `#__toes_category` AS `c` ON `c`.`category_id` = `e`.`copy_cat_category`
	LEFT JOIN `#__toes_division` AS `d` ON `d`.`division_id` = `e`.`copy_cat_division`
	LEFT JOIN `#__toes_entry_status` AS `es` ON `es`.`entry_status_id` = `e`.`status`
	LEFT JOIN `#__toes_show_class` AS `sc` ON `sc`.`show_class_id` = `e`.`entry_show_class`
...
if($is_alternative)
{
	...
	for($i = 0; $i < count($show_days); $i++)
	{
		...
		//echo nl2br(str_replace('#__', '#__', $query));die;
		$db->setQuery($query);
...
*/
$pdf->Output( $show_id . '_show_scheduling_summary.pdf', 'I');
//============================================================+
// END OF FILE                                                
//============================================================+
