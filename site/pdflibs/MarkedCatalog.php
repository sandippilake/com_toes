...
// spiderweb commented
$query = "SELECT `e`.* FROM `#__ticapp_entry` as `e` 
WHERE e.entry_show_id  = {$show_id}";

$db->setQuery($query);
$catalog_entries = $db->loadObjectList();

$showentries = array();
$classes = array('Kitten', 'Cat', 'Alter','HHP Kitten', 'HHP');
foreach ($catalog_entries as $i =>$entry1) {
	//echo '<pre>'; print_r($entry1); echo '</pre>';die;
	// spider array
	$show_class = str_replace('LH', '', $entry1->entry_competitive_class_name);
	$show_class = str_replace('SH', '', $show_class);
	$show_class = trim($show_class);

	$result_data[$entry1->entry_catalog_number][$entry1->entry_ring_id] = $entry1->entry_result_id_color;
	if($entry1->entry_result_id_division) {
		$division_array[$entry1->entry_competitive_class_id][$entry1->entry_breed_id][$entry1->entry_category_id][$entry1->entry_division_id][$entry1->entry_ring_id][$entry1->entry_result_id_division] = $entry1->entry_catalog_number;
	}
	$breed_array[$entry1->entry_competitive_class_id][$entry1->entry_breed_id][$entry1->entry_ring_id][$entry1->entry_result_id_breed] = $entry1->entry_catalog_number;
	if($entry1->entry_result_id_final != '')
	{
		if($rings_by_id[$entry1->entry_ring_id]->ring_format == 'Speciality') {
			$final_best_newarray[$entry1->entry_competitive_class_name][$entry1->entry_ring_id][$entry1->entry_result_id_final] = $entry1->entry_catalog_number;
		} else {
			if(in_array($show_class, $classes)){
				// $final_best_array[$entry1->entry_competitive_class_id][$entry1->entry_ring_id][$entry1->entry_result_id_final] = $entry1->entry_catalog_number;
				$final_best_newarray[$show_class][$entry1->entry_ring_id][$entry1->entry_result_id_final] = $entry1->entry_catalog_number; 
			}
		}
	}
	if($rings_by_id[$entry1->entry_ring_id]->ring_format == 'Speciality') {
		if($entry1->entry_result_id_color > 0 || $entry1->entry_result_id_color == "."  || $entry1->entry_result_id_color == "/") {
			if(isset($final_best_newarray[$entry1->entry_competitive_class_name][$entry1->entry_ring_id]['count'])) {
				$final_best_newarray[$entry1->entry_competitive_class_name][$entry1->entry_ring_id]['count']++;
			} else {
				$final_best_newarray[$entry1->entry_competitive_class_name][$entry1->entry_ring_id]['count'] = 1;
			}
		}
	} else {
		if($entry1->entry_result_id_color > 0 || $entry1->entry_result_id_color == "."  || $entry1->entry_result_id_color == "/") {
			if(isset($final_best_newarray[$show_class][$entry1->entry_ring_id]['count'])) {
				$final_best_newarray[$show_class][$entry1->entry_ring_id]['count']++;
			} else {
				$final_best_newarray[$show_class][$entry1->entry_ring_id]['count'] = 1;
			}
		}
	}
}

//var_dump($final_best_newarray);die;
$temp_entries = array();
$showday_entries = array();
				...
				foreach ($show_day_rings[$show_days[0]->show_day_id] as $first_show_day) {
					if ($show->show_format != 'Alternative') {
						if (isset($show_day_entries[$show_days[0]->show_day_id][$entry->catalog_number])) {
							if(isset($result_data[$entry->catalog_number][$first_show_day->ring_id])) {
								$entry_block .= '<td align="left" valign="top">'.$result_data[$entry->catalog_number][$first_show_day->ring_id].'</td>';
							} else {
						...
						if (isset($show_day_entries[$show_days[0]->show_day_id][$entry->catalog_number])) {
							...
							{
								if(isset($result_data[$entry->catalog_number][$first_show_day->ring_id])) {
									$entry_block .= '<td align="' . (($first_show_day->ring_timing == 1) ? 'left' : 'right') . '" valign="top">'.$result_data[$entry->catalog_number][$first_show_day->ring_id].'</td>';
								} else {
				...
				foreach ($show_day_rings[$show_days[1]->show_day_id] as $second_show_day) {
					if ($show->show_format != 'Alternative') {
						if (isset($show_day_entries[$show_days[1]->show_day_id][$entry->catalog_number])) {
							if(isset($result_data[$entry->catalog_number][$second_show_day->ring_id])) {
								$entry_block .= '<td align="right" valign="top">'.$result_data[$entry->catalog_number][$second_show_day->ring_id].'</td>';
							} else {
						...
						if (isset($show_day_entries[$show_days[1]->show_day_id][$entry->catalog_number])) {
							if (($second_show_day->ring_timing == 1 && $showday_entries[$show_days[1]->show_day_id][$entry->catalog_number]->entry_participates_AM) || ($second_show_day->ring_timing == 2 && $showday_entries[$show_days[1]->show_day_id][$entry->catalog_number]->entry_participates_PM)) {
								if(isset($result_data[$entry->catalog_number][$second_show_day->ring_id])) {
									$entry_block .= '<td align="' . (($second_show_day->ring_timing == 1) ? 'left' : 'right') . '" valign="top">'.$result_data[$entry->catalog_number][$second_show_day->ring_id].'</td>';
								} else {
		...
		while ($print_block > 0) {
			...
			if (in_array($entry->show_class, $skip_division_best)) {
				...
				for ($i = 1; $i <= $previous_division_entries; $i++) {
					...
					$best_division_block .= '<td width="30%" align="right">
                                        <table width="100%">
                                            <tr>
                                                ';
					foreach ($show_day_rings[$show_days[0]->show_day_id] as $first_show_day) {
						$best_of_division = $division_array[$entry->entry_show_class][$entry->copy_cat_breed][$entry->copy_cat_category][$entry->copy_cat_division][$first_show_day->ring_id][$i];
						if($best_of_division) {
							$best_division_block .= '<td colspan="2" align="' . (($show->show_format != 'Alternative' || $first_show_day->ring_timing == 1) ? 'left' : 'right') . '">'.$best_of_division.'</td>
                                        ';
						} else {
					...
					foreach ($show_day_rings[$show_days[1]->show_day_id] as $second_show_day) {
						$best_of_division = $division_array[$entry->entry_show_class][$entry->copy_cat_breed][$entry->copy_cat_category][$entry->copy_cat_division][$second_show_day->ring_id][$i];
						if($best_of_division) {
							$best_division_block .= '<td colspan="2" align="' . (($show->show_format == 'Alternative' && $second_show_day->ring_timing == 1) ? 'left' : 'right') . '">'.$best_of_division.'</td>';
						} else {
			...
			if (in_array($entry->show_class, $skip_breed_best)) {
				...
				for ($i = 1; $i <= $previous_breed_entries; $i++) {
					...
					foreach ($show_day_rings[$show_days[0]->show_day_id] as $first_show_day) {
						$best_of_breed = $breed_array[$entry->entry_show_class][$entry->copy_cat_breed][$first_show_day->ring_id][$i];
						if($best_of_breed) {
							$best_breed_block .= '<td colspan="2" align="' . (($show->show_format != 'Alternative' || $first_show_day->ring_timing == 1) ? 'left' : 'right') . '">'.$best_of_breed.' </td>';
						} else  {
					...
					foreach ($show_day_rings[$show_days[1]->show_day_id] as $second_show_day) {
						$best_of_breed = $breed_array[$entry->entry_show_class][$entry->copy_cat_breed][$second_show_day->ring_id][$i];
						if($best_of_breed) {
							$best_breed_block .= '<td colspan="2" align="' . (($show->show_format == 'Alternative' && $second_show_day->ring_timing == 1) ? 'left' : 'right') . '">'.$best_of_breed.'</td>';
						} else {
	...
	foreach ($entries as $entry) {
		...
		if ($previous_catalog_number != $entry->catalog_number) {
			...
			if ($entry->show_class != 'Ex Only') {
				...
				// spiderweb commented color block
				...
				if ($show->show_format != 'Alternative') {
					...
					foreach ($show_days as $show_day) {
						...
						//echo '<pre>'; print_r($result_data[$entry->catalog_number][$pr_show_day->ring_id]); echo '</pre>';die;
							
							if (isset($show_day_entries[$show_day->show_day_id][$entry->catalog_number]))
								if(isset($result_data[$entry->catalog_number][$pr_show_day->ring_id]))
								{
									$entry_block .= '<td align="left" valign="top">'.$result_data[$entry->catalog_number][$pr_show_day->ring_id].' </td>';
								} else {
									$entry_block .= '<td align="left" valign="top">____</td>';
								}
							else
					...
					foreach ($show_days as $show_day) {
						...
						foreach ($show_day_rings[$show_day->show_day_id] as $pr_show_day) {
							//spiderweb commented
						/*	$query = "select entry_result_id_color from tica.tica2019_ticapp_entry 
							where entry_toes_entry_id = ".$entry->entry_id." and `entry_show_id`=".$entry->show_id." and `entry_ring_id`=". $pr_show_day->ring_id;
							$db->setQuery($query);
							...
							if (isset($show_day_entries[$show_day->show_day_id][$entry->catalog_number])) {
								if (($pr_show_day->ring_timing == 1 && $showday_entries[$show_day->show_day_id][$entry->catalog_number]->entry_participates_AM) || ($pr_show_day->ring_timing == 2 && $showday_entries[$show_day->show_day_id][$entry->catalog_number]->entry_participates_PM))
									if(isset($result_data[$entry->catalog_number][$pr_show_day->ring_id]))
								{
									$entry_block .= '<td align="' . (($pr_show_day->ring_timing == 1) ? 'left' : 'right') . '" valign="top">'.$result_data[$entry->catalog_number][$pr_show_day->ring_id].'</td>';
								}else{
									$entry_block .= '<td align="' . (($pr_show_day->ring_timing == 1) ? 'left' : 'right') . '" valign="top">'.$color_id.'</td>';
								}
								else
						...
						$best_division_block .= '<td width="' . (count($show_days) == 3 ? '20%' : '50%') . '">
                                            <table>
                                                <tr>
                                                    ';
						foreach ($show_day_rings[$show_day->show_day_id] as $pr_show_day) {
							$best_of_division = $division_array[$entry->entry_show_class][$entry->copy_cat_breed][$entry->copy_cat_category][$entry->copy_cat_division][$pr_show_day->ring_id][$i];
							if($best_of_division) {
								$best_division_block .= '<td colspan="2" align="' . (($show->show_format != 'Alternative' || $pr_show_day->ring_timing == 1) ? 'left' : 'right') . '">'.$best_of_division.'</td>';
							} else {
			...
			if (in_array($entry->show_class, $skip_breed_best)) {
				...
				for ($i = 1; $i <= $previous_breed_entries; $i++) {
					...
					foreach ($show_days as $show_day) {
						...
						foreach ($show_day_rings[$show_day->show_day_id] as $pr_show_day) {
							// dhanu
							$best_of_breed = $breed_array[$entry->entry_show_class][$entry->copy_cat_breed][$pr_show_day->ring_id][$i];
						
							if($best_of_breed) {
								$best_breed_block .= '<td colspan="2" align="' . (($show->show_format != 'Alternative' || $pr_show_day->ring_timing == 1) ? 'left' : 'right') . '">'.$best_of_breed.'</td>';
							} else {
...
foreach ($final_entries as $ring_number => $cng_entries) {
	...
	foreach ($congress_entries as $entry) {
		...
		if ($previous_catalog_number != $entry->catalog_number) {
			...
			$entry_block .= '<td width="' . (100 - (count($rings) * 15)) . '%"><table>';
			...
			foreach ($rings as $ring) {
				if (in_array($entry->catalog_number, $cngrs_entries_by_ring_id[$ring->ring_id])) {
					...
					$congress_color_data1 = $result_data[$entry->catalog_number][$ring->ring_id];
				//var_dump($congress_color_data1);die;
					// dhanu congress color 
					$entry_block .= '<td width="15%">
										<table width="100%">
											<tr>';
											if($congress_color_data1)
											{
												$entry_block .= '<td align="left" valign="top">'.$congress_color_data1.'</td>';
											}
											else{
												$entry_block .= '<td align="left" valign="top">________</td>';
											}
												$entry_block .= '</tr>
										</table>
									</td>';
		...
		while ($print_block > 0) {
			...
			if ($entry->show_class != 'Ex Only' && in_array($entry->show_class, $skip_division_best)) {
				...
				for ($i = 1; $i <= $previous_division_entries; $i++) {
					...
					foreach ($rings as $ring) {
						//rahul congress best of division
					//	var_dump($ring);
						
						$congress_best_of_division1 = $division_array[$entry->entry_show_class][$entry->copy_cat_breed][$entry->copy_cat_division][$ring->ring_id][$i];
				
						$judge_block .= '<td width="15%">
                                            <table width="100%">
												<tr>';
												if($congress_best_of_division1)
												{
													$judge_block .= '<td align="left">'.$congress_best_of_division1.'</td>';
												}
												else{
													$judge_block .= '<td align="left">__________</td>';
												}
						$judge_block .= '</tr>						
                                            </table>
                                        </td>';
					...
					if (!in_array($class, $new_breed_classes)) {
						for ($j = 1; $j <= 10; $j++) {
							...
							foreach ($rings as $ring) {
								...
								$class_final_block .= '<td align="center">';
								if($ring->format == 'Specialty') {
									if($final_best_newarray['LH '.$class][$ring->ring_id][$j] != '') {
										$final_best_array1 = $final_best_newarray['LH '.$class][$ring->ring_id][$j];
										$class_final_block .= $final_best_array1;
									} else{
										$class_final_block .= '__________';
									}
									$class_final_block .= '</td>';	
									$class_final_block .= '<td align="center">';
									if($final_best_newarray['SH '.$class][$ring->ring_id][$j] != '') {
										$final_best_array1 = $final_best_newarray['SH '.$class][$ring->ring_id][$j];
										$class_final_block .= $final_best_array1;
									} else {
										$class_final_block .= '__________';
									}
								} else {
									if($final_best_newarray[$class][$ring->ring_id][$j] != '') {
										$final_best_array1 = $final_best_newarray[$class][$ring->ring_id][$j];
										$class_final_block .= $final_best_array1;
									} else{
										$class_final_block .= '__________';
									}
						...
						foreach ($rings as $ring) {
							...
							$class_final_block .= '<td align="center">';
							if($ring->format == 'Specialty') {
								if(isset($final_best_newarray['LH '.$class][$ring->ring_id]['count']) 
									&& $final_best_newarray['LH '.$class][$ring->ring_id]['count'] != '')
								{
									$final_best_array1 = $final_best_newarray['LH '.$class][$ring->ring_id]['count'];
									$class_final_block .= $final_best_array1;
								} else {
									$class_final_block .= '__________';
								}
								$class_final_block .= '</td>';	
								$class_final_block .= '<td align="center">';
								if(isset($final_best_newarray['SH '.$class][$ring->ring_id]['count']) 
									&& $final_best_newarray['SH '.$class][$ring->ring_id]['count'] != '')
								{
									$final_best_array1 = $final_best_newarray['SH '.$class][$ring->ring_id]['count'];
									$class_final_block .= $final_best_array1;
								} else {
									$class_final_block .= '__________';
								}
							} else {
								if(isset($final_best_newarray[$class][$ring->ring_id]['count']) 
									&& $final_best_newarray[$class][$ring->ring_id]['count'] != '')
								{
									$final_best_array1 = $final_best_newarray[$class][$ring->ring_id]['count'];
									$class_final_block .= $final_best_array1;
								} else {
									$class_final_block .= '__________';
								}
...
*/
$pdf->Output( $show_id .'_mastercatalog.pdf', 'I');
//============================================================+
// END OF FILE
//============================================================+
