...
if($is_alternative)
{
	...
	{
		if( !isset($show_class_divisions[$smry->show_class]) || !isset($show_class_divisions[$smry->show_class][$smry->category]) || !in_array($smry->category.'-'.$smry->division_name,$show_class_divisions[$smry->show_class][$smry->category])) {
				...
				if(isset($show_class_divisions[$show_class]))
				{
					foreach($show_class_divisions[$show_class] as $category_name => $category)
					{
						...
						foreach($category as $division_name)
						{
							...
							for($i = 0; $i < count($show_days); $i++)
							{
								$am_cnt = isset($hhp_am_summary[$show_class][$division_name][$show_days[$i]->show_day_id])?$hhp_am_summary[$show_class][$division_name][$show_days[$i]->show_day_id]:0;
								$pm_cnt = isset($hhp_pm_summary[$show_class][$division_name][$show_days[$i]->show_day_id])?$hhp_pm_summary[$show_class][$division_name][$show_days[$i]->show_day_id]:0;
