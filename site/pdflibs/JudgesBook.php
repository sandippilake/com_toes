...
foreach($temp_rings as $ring) {
	$rings[$ring->ring_show_day . $ring->ring_timing . $ring->ring_number] = $ring;
		...
		$fp =fopen(JPATH_BASE.'/judge_rings.txt','a+');
		ob_start();		
		var_dump($rings);
		$str = ob_get_clean();
		fwrite($fp,"This is rings dump\n".$str."\n");
		fclose($fp);
...
$pdf->SetCreator("TICA");
$pdf->SetTitle(JText::_('COM_TOES_JUDGES_BOOK'));
...
foreach($final_entries as $ring_number => $ring_entries) {
	/* if($j>1)
	  break; */
	$fp =fopen(JPATH_BASE.'/judge_rings.txt','a+');	 
	fwrite($fp,"This is current ring number\n".$ring_number."\n");
	fclose($fp);
	...
	if ($cover_copy) {
		...
		$front_page .= '<div style="">' . JText::_('COM_TOES_JUDGES_BOOK_AB') . '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' . JText::_('COM_TOES_JUDGES_BOOK_LH') . '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' . JText::_('COM_TOES_JUDGES_BOOK_SH') . '</div><br/>';
		...
		$pdf->writeHTMLCell($w, 0, $x, 5, $front_page, 0, true, false, false, false, false);
		$html .= $front_page;
	...
	$judge_rings = TOESHelper::judge_rings($show->show_id, $rings[$ring_number]->ring_judge);
	...
	$fp =fopen(JPATH_BASE.'/judge_rings.txt','a+');	 
	fwrite($fp,"This is ring judge id\n".$rings[$ring_number]->ring_judge."\n");
	fclose($fp);
	...
	if ($judge_rings) {
		...
		fwrite($fp,"There are judge rings \n");
	...
	fwrite($fp , "Count is ".$count."\n");
	fwrite($fp , "Judge fee is ".$judge_fee."\n");
	fclose($fp);
	...
	$expense_block .= '<tr>';
	$expense_block .= '<td align="right" colspan="3">' . JText::_('COM_TOES_TOTAL') . ' : ' . $count . ' X ' . $judge_fee . ' USD</td>';
...
$fp = fopen(JPATH_BASE.'/judgesbook_'.$show_id.'.html','w+');
fwrite($fp,$html);
fclose($fp);
