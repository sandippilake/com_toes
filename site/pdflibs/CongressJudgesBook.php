...
$fp =fopen(JPATH_BASE.'/congressjudgeabook_entries.txt','a+');
ob_start();		
var_dump($rings);
$str = ob_get_clean();
...
$fp =fopen(JPATH_BASE.'/final_entries.txt','a+');
		ob_start();		
		var_dump($final_entries);
		$str = ob_get_clean();
		fwrite($fp,"This is final_entries dump\n".$str."\n");
		fclose($fp);
    ...
    foreach($entries as $entry)
    {
        ...
        while ($print_block > 0) 
        {
            if($previous_catalog_number != $entry->catalog_number)
            {
				...
				/*
				$pdf->writeHTMLCell( $w, 0, $x, $pdf->GetY(), 'previous_breed:'. $previous_breed, 0, true, false, false, false, '');
				$pdf->writeHTMLCell( $w, 0, $x, $pdf->GetY(), 'previous_division:'. $previous_division, 0, true, false, false, false, '');
				$pdf->writeHTMLCell( $w, 0, $x, $pdf->GetY(), 'this breed:'. $entry->breed_abbreviation, 0, true, false, false, false, '');
				$pdf->writeHTMLCell( $w, 0, $x, $pdf->GetY(), 'this division:'. $entry->catalog_division, 0, true, false, false, false, '');
				$pdf->writeHTMLCell( $w, 0, $x, $pdf->GetY(), 'previous_color:'. $previous_color, 0, true, false, false, false, '');
				$pdf->writeHTMLCell( $w, 0, $x, $pdf->GetY(), 'this color:'. $entry->judges_book_color, 0, true, false, false, false, '');
				...
				*/
                ...
                {
					//
					//$pdf->writeHTMLCell( $w, 0, $x, $pdf->GetY(), 'In if:', 0, true, false, false, false, '');
			...
			$next = $cur + 1;
			/*
			$pdf->writeHTMLCell($w, 0, $x, $pdf->GetY(), '$next'.$next, 0, true, false, false, false, '');
			$pdf->writeHTMLCell($w, 0, $x, $pdf->GetY(), 'count($entries)'.count($entries), 0, true, false, false, false, '');
			
			
			$pdf->writeHTMLCell($w, 0, $x, $pdf->GetY(), 'this breed ->'.$entry->breed_abbreviation, 0, true, false, false, false, '');
			$pdf->writeHTMLCell($w, 0, $x, $pdf->GetY(), 'next breed'.@$entries[$next]->breed_abbreviation, 0, true, false, false, false, '');
			$pdf->writeHTMLCell($w, 0, $x, $pdf->GetY(), 'this division'.$entry->catalog_division, 0, true, false, false, false, '');
			$pdf->writeHTMLCell($w, 0, $x, $pdf->GetY(), 'next division'.@$entries[$next]->catalog_division, 0, true, false, false, false, '');
			*/
			if (in_array($entry->show_class, $skip_division_best)) {
				...
				//$pdf->writeHTMLCell($w, 0, $x, $pdf->GetY(), 'This is jusdge block', 0, true, false, false, false, '');
                if($show->show_print_extra_line_at_end_of_color_class_in_judges_book)
                {
					$entry_block = '<table style="text-align:center;padding:5px;margin:0;">
										<tr>
...
if($cover_copy)
{
	...
	$pdf->Output( $show_id .'_congressjudgesbook_cover.pdf', 'I');
}
else
{
	...
	header("Content-type: application/pdf");
	$pdf->Output( $show_id .'_congressjudgesbook.pdf', 'I');
}
