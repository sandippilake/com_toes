...
foreach($show_days as $show_day) {
	...
	//$ring->ring_id = 'AB';
	// sandy added 
	$ring->ring_id = 'AB_'.$show_day->show_day_id;
	...
	//$ring->ring_id = 'SP';
	// sandy added
	$ring->ring_id = 'SP_'.$show_day->show_day_id;
	$ring->ring_show = $show_id;
	$ring->ring_show_day = $show_day->show_day_id;
	$ring->ring_timing = 1;
...
$pdf->SetCreator("TICA");
$pdf->SetTitle(JText::_('COM_TOES_JUDGES_BOOK_IN_A4'));
...
foreach($final_entries as $ring_number => $ring_entries) {
	...
	if ($cover_copy) {
		...
		if($params->get('use_logo_for_pdf')) {
			...
			$front_page .= '<div style="text-align:center;"> </div>';
		}
		...
		$front_page .= '<div style="text-align:center;">' . JText::_('COM_TOES_JUDGES_BOOK_JUDGE') . ' : ' . $pdf->judge_name . '</div>';
		$front_page .= '<div style="text-align:center;">' . JText::_('COM_TOES_JUDGES_BOOK_CLUB') . ' : ' . $show->club_name . '</div>';
		$front_page .= '<div style="text-align:center;">' . JText::_('COM_TOES_JUDGES_BOOK_PLACE') . ' : ' . $show->Show_location . '</div>';
		$front_page .= '<div style="text-align:center;">' . JText::_('COM_TOES_JUDGES_BOOK_DATE') . ' : ' . $show_day_date . '</div>';
		$front_page .= '<div style="text-align:center;">' . JText::_('COM_TOES_JUDGES_BOOK_AB') . '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' . JText::_('COM_TOES_JUDGES_BOOK_LH') . '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' . JText::_('COM_TOES_JUDGES_BOOK_SH') . '</div><br/>';

		$front_page .= '<div style="text-align:center;">' . JText::_('COM_TOES_JUDGES_BOOK_AWARD_NOTE') . '</div><br/><br/>';
		$front_page .= '<div style="text-align:center;">' . JText::_('COM_TOES_JUDGES_BOOK_SIGN') . '</div><br/>';
		$front_page .= '<div style="text-align:center;">' . JText::_('COM_TOES_JUDGES_BOOK_RING_CLERK') . '</div>';
		$front_page .= '<div style="text-align:center;">' . JText::_('COM_TOES_JUDGES_BOOK_RING_ASS_CLERK') . '</div><br/><br/>';

		$front_page .= '<div style="text-align:center;">' . JText::_('COM_TOES_JUDGES_BOOK_NOTICE') . '</div>';

		$pdf->SetFont('cmunrm', '', 12);
		$pdf->writeHTMLCell($w, 0, $x, 0, $front_page, 0, true, false, false, false, false);
	...
	foreach($entries as $entry) {
		...
		if ($previous_class != $entry->show_class) {
			if ($previous_class != '') {
				$judge_page_number++;	
				if ($judge_page_number % 2 == 0) {
					$pdf->footer_x = $x;
					$pdf->AddPage();
					$x = 5;
					if($final_printed == true) {
						$final_printed = false;
					}
				} else {
					if($final_printed == true) {
			...
			if ($is_continuous)
				...
				$show_day_date = date('M d, Y', strtotime($show_days[$entry->show_day]->show_day_date));
			if ($is_alternative) {
				if ($rings[$ring_number]->ring_timing == 1)
					$show_day_date .= " AM";
				else
			...
			$show_class_block .= '<div style="text-align:center;">' . strtoupper($entry->show_class) . '</div></td>';
			...
			if ($previous_catalog_number != $entry->catalog_number) {
				...
				if ($previous_breed != $entry->breed_abbreviation || $previous_division != $entry->catalog_division || $previous_color != $entry->judges_book_color) {
					...
					if($show->show_print_division_title_in_judges_books)
					{
						...
						if ($previous_breed != $entry->breed_abbreviation || $previous_division != $entry->catalog_division) {
							$pdf->SetFont('ptsans', '', 10);
...
if ($cover_copy) {
	/*
	if (file_exists(TOES_PDF_PATH . DS . $show_id . DS . 'judgesbook_cover.pdf'))
		unlink(TOES_PDF_PATH . DS . $show_id . DS . 'judgesbook_cover.pdf');
	*/
	//Close and output PDF document
	//$pdf->Output(TOES_PDF_PATH . DS . $show_id . DS . 'judgesbook_cover.pdf', 'F');
	$pdf->Output( $show_id . '_judgesbook_cover.pdf', 'I');
} else {
	/*
	if (file_exists(TOES_PDF_PATH . DS . $show_id . DS . 'judgesbook.pdf'))
		unlink(TOES_PDF_PATH . DS . $show_id . DS . 'judgesbook.pdf');
	*/
	//Close and output PDF document
	//$pdf->Output(TOES_PDF_PATH . DS . $show_id . DS . 'judgesbook.pdf', 'F');
	$pdf->Output( $show_id . '_judgesbook.pdf', 'I');
}
//============================================================+
// END OF FILE                                                
//============================================================+
