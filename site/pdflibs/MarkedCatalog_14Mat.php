...
//var_dump($show_days);die;


$show = TOESHelper::getShowDetails($show_id);
//echo '<pre>';print_r($show);echo '</pre>';die;
...
//echo '<pre>'; print_r($catalog_entries); echo '</pre>';die;
...
foreach ($catalog_entries as $i =>$entry1) {
	...
	$division_array[$entry1->entry_competitive_class_id][$entry1->entry_breed_id][$entry1->entry_division_id][$entry1->entry_ring_id][$entry1->entry_result_id_division] = $entry1->entry_catalog_number;
		...
		while ($print_block > 0) {
			...
			if (in_array($entry->show_class, $skip_division_best)) {
				...
				for ($i = 1; $i <= $previous_division_entries; $i++) {
					...
					foreach ($show_day_rings[$show_days[1]->show_day_id] as $second_show_day) {
						$best_of_division = $division_array[$entry->entry_show_class][$entry->copy_cat_breed][$entry->copy_cat_division][$second_show_day->ring_id][$i];
						if($best_of_division)
						{
			...
			if (in_array($entry->show_class, $skip_breed_best)) {
				...
				for ($i = 1; $i <= $previous_breed_entries; $i++) {
					...
					foreach ($show_day_rings[$show_days[0]->show_day_id] as $first_show_day) {
						...
						//$best_of_breed = $breed_array[$entry1->entry_show_class][$entry1->copy_cat_breed][$first_show_day->ring_id][$i];
						if($best_of_breed)
						{
					...
					foreach ($show_day_rings[$show_days[1]->show_day_id] as $second_show_day) {
						...
						//$best_of_breed = $breed_array[$entry1->entry_show_class][$entry1->copy_cat_breed][$ring->ring_id][$i];
							if($best_of_breed)
							{
							$best_breed_block .= '<td colspan="2" align="' . (($show->show_format == 'Alternative' && $second_show_day->ring_timing == 1) ? 'left' : 'right') . '">'.$best_of_breed.' k </td>';
							}
							{
...
//echo '<pre>'; print_r($entry); echo '</pre>';die;
						//spiderweb commented best of division block
						

						$best_of_division = $division_array[$entry->entry_show_class][$entry->copy_cat_breed][$entry->copy_cat_division][$pr_show_day->ring_id][$i];
							//var_dump($best_of_division);
							if($best_of_division)
							{
