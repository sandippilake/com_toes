...
$handle = fopen(TOES_PDF_PATH.DS.$show->show_id.DS.'precongressjudgesbook.txt', 'w');
...
foreach($final_entries as $ring_number=>$ring_entries)
{
	...
	if( $start_page <= $page_number && $page_number <= $end_page)
	{
		...
		$show_class_text = $rings[$ring_number]->ring_name;
		...
		$date = date('M-d',strtotime($show_days[$rings[$ring_number]->ring_show_day]->show_day_date));
		if($is_alternative)
		{
    ...
    foreach($entries as $entry)
    {
                ...
                ( isset($entries[$cur]) && ($entry->breed_abbreviation != $entries[$cur]->breed_abbreviation || $entry->catalog_division != $entries[$cur]->catalog_division)))
        {
			...
			if(!in_array($entry->show_class, $skip_breed_best) && ($cur == count($entries) && $previous_breed_entries == $previous_division_entries) || ($previous_breed_entries == $previous_division_entries && ($entry->breed_abbreviation != $entries[$cur]->breed_abbreviation)))
            {
