    ...
    switch(step)
    {
        ...
        case 'step1':
            ...
            if(cat_id)
            {
                jQuery.ajax({
                    url: 'index.php?option=com_toes&task=entry.step&tmpl=component',
                    data: 'cat_id='+cat_id+'&user_id='+user_id+'&step=step1_5'+'&parent_div='+parent_div+'&type='+type,
                    type: 'post',
                }).done(function(responseText){
					responseText = responseText.trim();
                    jQuery('#'+parent_div).html(responseText);
            ...
            break;
        case 'step1_5':
            // var docs = jQuery('#docs').val();
            // if(cat_id)
            // {
                jQuery.ajax({
                    url: 'index.php?option=com_toes&task=entry.step&tmpl=component',
                    data: '&user_id='+user_id+'&step=step2'+'&parent_div='+parent_div+'&type='+type+'&'+jQuery('#adminForm').serialize(),
                    type: 'post',
                }).done(function(responseText){
					responseText = responseText.trim();
                    jQuery('#'+parent_div).html(responseText);
...
function save_new_user()
{
    ...
    var city = jQuery('input[name=city_name]').val().toString();
    var zip = jQuery('input[name=zip]').val().toString();
   // var state = jQuery('input[name=state]').val().toString();
    var state = jQuery('input[name=state_name]').val().toString();
    //var country = jQuery('input[name=country]').val().toString();
    var country = jQuery('input[name=country_name]').val().toString();
    var phonenumber = jQuery('input[name=phonenumber]').val().toString();
    var tica_region = jQuery('select[name=tica_region]').val().toString();
...
function approve_documents(entry_id){
	 
    jQuery('a#approve_entry_'+entry_id).trigger('click');
    return true;
	
}
function reject_documents(entry_id){
	 jQuery('a#reject_entry_'+entry_id).trigger('click');
      
    return true;
	 
}
function eo_approve_documents(cat_id){
	jQuery.ajax({
		url: 'index.php?option=com_toes&task=executiveoffice.eo_approve_documents&tmpl=component',
		data: {cat_id:cat_id},
		type: 'post',
	}).done(function(responseText){
		responseText = responseText.trim();
